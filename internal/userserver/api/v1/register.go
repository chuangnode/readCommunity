...
func Register(c *gin.Context) {
	...
	if err := initialize.ValidateStruct(registerParams); err != nil {
		//validation.HandleValidatorError(c, err)
		errs, ok := err.(validator.ValidationErrors)
		if !ok {
			app.ResponseWithError(c, errcode.ErrValidation, "参数不正确", err.Error())
		} else {
			info := validation.RemoveTopStruct(errs.Translate(global.Trans))
			app.ResponseWithError(c, errcode.ErrValidation, "参数不正确", info)
		}
	...
	if err != nil {
		...
		zap.L().Info("register failed", zap.Error(err))
